    ...
    DbUtils.queryData(querySql, function (result) {
        ...
        if (result && result[0].count > 0) {
            ...
            if (req.body.isUpdate == 'N') {
                ...
                inserSql = 'UPDATE company_org m set m.company_desc = "' + req.body.mnueDesc + '",m.parent_code = "' + req.body.parentId + '",m.address_code="sz",m.is_tz=0 where m.company_code ="' + req.body.id+'"';
            }
            console.log(inserSql);
            DbUtils.queryData(inserSql, function (result) {
                data.status = 0;
