...
$(function () {
    ...
    $('#cost-list').on('click', function () {
       init();
        //获取费用类型数据
        getLevelTypeListData(1, utils.pageSize);
    });
    ...
    $('#cost-type-modify').on('click', function () {
        ...
        utils.ajaxSend({
            type: 'get',
            url: '/baixiu/modifyLevelTypeMaintenanceInfo',
            data: {
                costTypeCode: $('#cost_modify .cost_type_code_view').val(),
                costTypeDesc:$('#cost_type_desc_modify').val()
            },
            dataType: "json"
        }, function (data) {
            if(data.status == 0){
                $('#cost_modify .cost_type_desc_view').val($('#cost_type_desc_modify').val());
            }
        }, function (error) {
            alert(error);
        });
    ...
    $('.cost-search').on('click', function () {
        if (!$('#cost_modify .cost_type_code_view').val() && !$('#cost_modify .cost_type_desc_view').val()) {
            alert('请输入职位类型');
            return;
        }
        utils.ajaxSend({
            type: 'get',
            url: '/baixiu/searchLevelMaintenanceTypeInfo',
            data: {
                costTypeCode: $('#cost_modify .cost_type_code_view').val(),
            },
            dataType: "json"
    ...
    $('#cost-standard-add').on('click',function () {
        ...
        var costTypeDesc = $('#cost_add .cost_type_desc_view').val();//费用类型描述
        if(!costType){
            alert('请填写职位类型代码');
            return;
        }
    ...
    //获取公司类型数据getCompanyTypeListData   getLevelTypeListData
    getLevelTypeListData(1, utils.pageSize);
});
function init() {
...
function getLevelTypeListData(offset, pageSize) {
    utils.ajaxSend({
        type: 'get',
        url: '/baixiu/searchLevelTypeMaintenanceList',
        data: {offset, pageSize},
        dataType: "json"
    }, function (data) {
        var html = template('costsList', data);
        $('.levelMaintenance table tbody').html(html);
        $('.cost_type_edit').on('click', function () {
            $('.levelMaintenance .cost-tabs li.cost_list_li').removeClass('active');
            $('.levelMaintenance .cost-tabs li.cost_add_li').removeClass('active');
            $('.levelMaintenance .cost-tabs li.cost_modify_li').addClass('active');
            $('.levelMaintenance .cost-content div#cost_list').removeClass('active');
            $('.levelMaintenance .cost-content div#cost_add').removeClass('active');
            $('.levelMaintenance .cost-content div#cost_modify').addClass('active');
            ...
            $('#cost_modify .cost_type_desc_view').val(costTypeDesc);
            $('#cost_modify .cost_type_code_view').val(costTypeCode);
        });
        ...
        utils.pageList(data, $('.pages-nav'), function (currentPage, pageSize) {
            getLevelTypeListData(currentPage, pageSize);
        });
    }, function (error) {
...
function showInfo(type,obj) {
    if (type == 'FY') {
        ...
        $('#cost_modify .cost_type_desc_view').val(obj.innerHTML);
        $('#cost_modify .cost_type_code_view').val(obj.dataset.costCode);
    }
    $('.levelMaintenance .cost-manger-tree').modal('hide');
}
